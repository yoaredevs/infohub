...
-- HTTP GET compatível com Delta, Codex, Arceus X e executores PC
...
-- ======== CRIAÇÃO DAS ABAS (principais na frente, info no final) ========
...
-- ================================================================
-- ======== ABA: LEGIT (Speed + Hitbox Expander) ========
-- ================================================================
...
local espBox = true           -- highlight (chams)
local espBox2D = true         -- contorno 2D por fora do jogador
local espTracer = true
local espNome = true
local espVida = true
local espDist = true
local espCor = Color3.fromRGB(255, 80, 80)
local espTracerCor = Color3.fromRGB(255, 255, 255)
local espUsarCorTime = false
local espDistMax = 1000 -- studs
local espTracerMelhor = true  -- tracer rainbow no melhor jogador

local espObjetos = {}     -- [player] = { highlight, billboard, boxFrame, tracer, labels... }
local espConexoes = {}    -- conexões pra desconectar ao desligar
local espLoop
local espRenderLoop       -- loop por frame (box 2D + tracer)
local espMelhorLoop       -- loop de deteção do melhor jogador
local espMelhorJogador = nil
...
-- ---- DETECÇÃO DO MELHOR JOGADOR ----
-- Varre leaderstats procurando stats de "ranking" comuns em jogos de FPS/tiro,
-- com pesos: kills valem mais que level, level mais que wins, etc.
local ESP_STATS_PESO = {
    { padroes = { "kill", "kos", "knockout", "elimin", "frag" }, peso = 100 },
    { padroes = { "level", "lvl", "rank", "prestige" },          peso = 50 },
    { padroes = { "win", "vitoria", "victories" },               peso = 30 },
    { padroes = { "point", "score", "xp", "exp", "cash", "coin", "credit" }, peso = 1 },
}

local function pontuacaoDoJogador(jogador)
    ...
    for _, stat in ipairs(pasta:GetChildren()) do
        if stat:IsA("IntValue") or stat:IsA("NumberValue") then
            local nomeStat = string.lower(stat.Name)
            for _, grupo in ipairs(ESP_STATS_PESO) do
                for _, padrao in ipairs(grupo.padroes) do
                    if string.find(nomeStat, padrao, 1, true) then
                        total = total + (tonumber(stat.Value) or 0) * grupo.peso
                        break
                    end
                end
            end
...
local function detectarMelhorJogador()
    local melhor, melhorPontos = nil, 0
    for _, jogador in ipairs(Players:GetPlayers()) do
        if jogador ~= LocalPlayer then
            local pontos = pontuacaoDoJogador(jogador)
            if pontos > melhorPontos then
                melhor, melhorPontos = jogador, pontos
            end
        end
    end
    return melhor
...
-- cor rainbow ciclando pelo matiz
local function corRainbow()
    return Color3.fromHSV((tick() % 4) / 4, 1, 1)
end

-- calcula a box 2D projetando os 8 cantos do bounding box do personagem
local function calcularBox2D(char, root)
    local tamanho = Vector3.new(2, 3, 2) -- meio-tamanho aproximado do avatar
    ...
    for x = -1, 1, 2 do
        for y = -1, 1, 2 do
            for z = -1, 1, 2 do
                local canto = cf * Vector3.new(tamanho.X * x, tamanho.Y * y, tamanho.Z * z)
                local pos, naTela = Camera:WorldToViewportPoint(canto)
                if pos.Z > 0 then
                    visivel = visivel or naTela
    ...
    if not visivel or minX == math.huge then return nil end
...
local function criarESPPara(jogador)
    ...
    -- BOX 2D: contorno retangular por fora do jogador (desenhado na tela)
    local boxFrame = Instance.new("Frame")
    boxFrame.Name = "ESPBox2D_" .. jogador.Name
    boxFrame.BackgroundTransparency = 1
    boxFrame.BorderSizePixel = 0
    boxFrame.Visible = false
    boxFrame.Parent = espTela
    local boxStroke = Instance.new("UIStroke")
    boxStroke.Color = cor
    boxStroke.Thickness = 1.5
    boxStroke.Parent = boxFrame

    -- TRACER: linha da base da tela até o jogador (Frame fino rotacionado)
    local tracer = Instance.new("Frame")
    tracer.Name = "ESPTracer_" .. jogador.Name
    tracer.AnchorPoint = Vector2.new(0.5, 0.5)
    tracer.BackgroundColor3 = espTracerCor
    tracer.BorderSizePixel = 0
    tracer.Visible = false
    tracer.Parent = espTela
...
-- ---- RENDER POR FRAME (box 2D + tracer, precisa ser suave) ----
local function renderizarESP()
    local viewport = Camera.ViewportSize
    local baseTracer = Vector2.new(viewport.X / 2, viewport.Y) -- base inferior central

    for jogador, obj in pairs(espObjetos) do
        local char = jogador.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        ...
        if root then
            ...
            local dist = meuRoot and (root.Position - meuRoot.Position).Magnitude or 0
            mostrar = dist <= espDistMax
        end

        -- BOX 2D
        if mostrar and espBox2D then
            local x, y, w, h = calcularBox2D(char, root)
            if x then
                obj.boxFrame.Visible = true
                obj.boxFrame.Position = UDim2.new(0, x, 0, y)
                obj.boxFrame.Size = UDim2.new(0, w, 0, h)
                obj.boxStroke.Color = corDoJogador(jogador)
            else
        ...
        -- TRACER
        if mostrar and espTracer then
            local pos, naTela = Camera:WorldToViewportPoint(root.Position)
            if naTela and pos.Z > 0 then
                local destino = Vector2.new(pos.X, pos.Y)
                local delta = destino - baseTracer
                local comprimento = delta.Magnitude
                local meio = baseTracer + delta / 2

                obj.tracer.Visible = true
                obj.tracer.Position = UDim2.new(0, meio.X, 0, meio.Y)
                obj.tracer.Size = UDim2.new(0, comprimento, 0, 1.5)
                obj.tracer.Rotation = math.deg(math.atan2(delta.Y, delta.X))

                -- melhor jogador do servidor ganha tracer rainbow
                if espTracerMelhor and jogador == espMelhorJogador then
                    obj.tracer.BackgroundColor3 = corRainbow()
                    obj.tracer.Size = UDim2.new(0, comprimento, 0, 2.5)
                else
                    obj.tracer.BackgroundColor3 = espTracerCor
                end
            else
...
local function ligarESP()
    ...
    -- loop por frame: box 2D e tracers precisam acompanhar a câmera
    espTela.Enabled = true
    espRenderLoop = RunService.RenderStepped:Connect(function()
        pcall(renderizarESP)
    end)

    -- deteta o melhor jogador a cada 5 segundos (leaderstats mudam devagar)
    espMelhorLoop = task.spawn(function()
        while espAtivo do
            pcall(function() espMelhorJogador = detectarMelhorJogador() end)
            task.wait(5)
        end
    end)
end
...
local function desligarESP()
    ...
    if espRenderLoop then
        pcall(function() espRenderLoop:Disconnect() end)
        espRenderLoop = nil
    end
    if espMelhorLoop then
        pcall(function() task.cancel(espMelhorLoop) end)
        espMelhorLoop = nil
    end
    espMelhorJogador = nil
    espTela.Enabled = false
...
TabESP:CreateToggle({
    Name = "Box 2D (contorno por fora)",
    CurrentValue = true,
    Flag = "esp_box2d",
    Callback = function(valor) espBox2D = valor end
})
...
TabESP:CreateToggle({
    ...
    CurrentValue = true,
    Flag = "esp_tracer",
    Callback = function(valor) espTracer = valor end
})
...
TabESP:CreateColorPicker({
    ...
    Color = espTracerCor,
    Flag = "esp_tracer_cor",
    Callback = function(cor) espTracerCor = cor end
})

TabESP:CreateToggle({
    Name = "Tracer rainbow no melhor jogador",
    CurrentValue = true,
    Flag = "esp_tracer_melhor",
    Callback = function(valor)
        espTracerMelhor = valor
        if valor and espAtivo then
            espMelhorJogador = detectarMelhorJogador()
            if espMelhorJogador then
                notificar("ESP", "Melhor jogador: " .. espMelhorJogador.DisplayName, 3, "trophy")
            else
                notificar("ESP", "Nenhum leaderstat de ranking encontrado neste jogo.", 3, "info")
            end
        end
    end
})
...
local function aplicarFpsBoost()
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        -- não mexe em nada dos personagens (ESP continua intacto)
...
-- ================================================================
-- ======== ABA LEGIT: STAFF / ADMIN DETECTOR ========
-- (definido aqui pra enxergar todas as funções de desligamento acima)
-- ================================================================
...
local function staffDesligarTudo()
    ...
    for _, nomeGui in ipairs({ "InfoHubIntro", "InfoHubErro", "InfoHubAim", "InfoHubESP" }) do
        pcall(function()
            local g = getParent():FindFirstChild(nomeGui)
...
-- ================================================================
-- ======== ABAS DE INFORMAÇÃO (Jogador, Servidor, Tempo) ========
-- ================================================================
...
-- ================================================================
-- ======== DESTROY GUI (fim da aba Utilidades) ========
-- ================================================================
